...
export function getFilterSiblings(allFilters, filterParent, filterKey) {
  function findSiblings(filterPathObj, ancestors) {
    // early exit if no ancestors or no filters to search
     if (!filterPathObj || Object.keys(filterPathObj).length === 0 || ancestors.length === 0) {
        return {};
      };
...
// Treat empty/placeholder values as "empty" so card fields can hide them.
