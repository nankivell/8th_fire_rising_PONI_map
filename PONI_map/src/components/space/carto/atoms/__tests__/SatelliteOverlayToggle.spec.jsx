...
describe("<SatelliteOverlayToggle />", () => {
  it("shows the option to switch to satellite by default", () => {
    render(<SatelliteOverlayToggle toggleSatellite={vi.fn()} />);
  ...
  it("calls toggleNative when clicking the native layer button", () => {
    const mockNative = vi.fn();
    render(<SatelliteOverlayToggle toggleNative={mockNative} />);
    const btn = screen.getByRole("button", { name: /native land/i });
    fireEvent.click(btn);
